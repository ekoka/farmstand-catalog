...
export class HALResource {
    ...
    get _curies(){ return this._links.curies || [] }

    get _namespaces(){ return this._curies.map( c => c['name'] ) }

    _namespacedRel(relation) {
        if ( this._namespaces.length===0 ) return

        const key = Object.keys(this._links).find( rel => {
            // skipping the 'curies' relation
            if (rel==='curies') return
            return this._namespaces.find( ns => rel===ns + ':' + relation )
        })
    ...
    // alias of key()
    prop(p){ return this.resource[p] }

    // alias of prop()
    key(k){ return this.resource[k] }

    embedded(prop){ return this._embedded[prop] }

    get isPartial(){ return this.resource._partial ? true : false }
}
